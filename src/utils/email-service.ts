  ...
  try {
    ...
    const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        from: { email: 'bookings@dandeliadventures.com', name: 'Nature Loves adventures' },
...
export const formatBookingEmail = (bookingData: any): string => {
  ...
  return `
    ...
    <body>
      <div class="container">
        <div class="header">
          <h1>Nature Loves adventures - Booking Confirmation</h1>
        </div>
        <div class="content">
          <p>Dear ${bookingData.name},</p>
          <p>Thank you for booking with Nature Loves adventures. We're excited to host you!</p>
          ...
          <p>Warm regards,<br>The Nature Loves adventures Team</p>
        </div>
        <div class="footer">
          <p>© ${new Date().getFullYear()} Nature Loves adventures. All rights reserved.</p>
...
export const formatBookingSMS = (bookingData: any): string => {
  return `Thank you for booking with Nature Loves adventures! Your booking for ${bookingData.date ? bookingData.date.toLocaleDateString() : 'your selected date'} is confirmed (Ref: ${bookingData.paymentTransactionId || 'Processing'}). For assistance, call +91 8904704234.`;
};
...
export const sendBookingConfirmations = async (bookingData: any, transactionId: string): Promise<boolean> => {
  try {
    ...
    // Send to admin (Nature Loves adventures)
    await sendEmail({
